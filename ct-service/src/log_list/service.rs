...
impl CachingLogListService {
    ...
    pub async fn get_all_operator_names(&self) -> Result<Vec<String>, LogListError> {
        Ok(self.client.fetch_log_list().await?.operators.iter().map(|operator| operator.name.clone()).collect::<Vec<String>>())
    }
}
