...
async fn main() -> Result<()> {
    ...
    let host = env::var("HOST").unwrap_or_else(|_| "http://127.0.0.1".to_string());
    let port = env::var("PORT").unwrap_or_else(|_| "50524".to_string());

    let key = KeyChain.get_or_create_signing_key(SERVICE_ID).unwrap();
    let signing_key = Box::leak(Box::new(Ed25519(Box::new(key))));
    let (tx, rx) = mpsc::channel(1024);

    let scheduler = Arc::new(scheduler::Scheduler::new(15, 60));
    // running the client_handle
    let prism_client_runner = tokio::spawn(prism_client::run_prism_client(format!("{}:{}",host, port), rx, signing_key));
    ...
    tokio::select! {
        res_client = prism_client_runner => {
            match res_client.map_err(|e| anyhow!("Prism client has stopped unexpectedly with error: {}", e))? {
                Ok(()) => {
